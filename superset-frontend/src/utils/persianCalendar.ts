...
export const ensureJalaliDayjsPlugin = (): boolean => {
  ...
  try {
    ...
    const jalaliModule = require('dayjs-jalali');
    const jalaliPlugin: DayjsPlugin | undefined =
      typeof jalaliModule === 'function'
        ? (jalaliModule as DayjsPlugin)
        : (jalaliModule as { default?: DayjsPlugin }).default;
    if (jalaliPlugin) {
