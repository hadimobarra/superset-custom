...
export function PersianCalendarFrame({ onChange, value }: FrameComponentProps) {
  ...
  const customRangeSummary = useMemo(() => {
    ...
    return toPersianDigits(summary);
  ...
  const currentDateDisplay = useMemo(() => {
    ...
    return toPersianDigits(baseText);
  }, [currentPersianDate]);

  const titleText = PERSIAN_TEXT.title;
  const selectRangeText = PERSIAN_TEXT.selectTimeRange;
  const chooseCustomRangeText = PERSIAN_TEXT.chooseCustomRange;
  const setStartTodayText = PERSIAN_TEXT.startToToday;
  const setEndTodayText = PERSIAN_TEXT.endToToday;
  const selectedRangeLabel = PERSIAN_TEXT.selectedRangeLabel;
  const currentDateLabel = PERSIAN_TEXT.currentDateLabel;
